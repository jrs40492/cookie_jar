...
if (isset($_POST['name1']) && isset($_POST['name2']) && isset($_POST['number']) && isset($_POST['email']) && isset($_POST['address']) && isset($_POST['order']) && isset($_POST['captcha'])) {
  ...
  $total = 0;
  $orderList = '<table style="width: 100%;"><thead><tr><th>Kind</th><th>Amount @ Cost</th><th>Total</th></tr></thead><tbody>';
  for ($x = 0; $x < count($orderArray); $x++) {
    if ($orderArray[$x][3] != 0) {
      ...
      $orderList .= "<tr><td>".$orderArray[$x][1]."</td><td>".$orderArray[$x][3]." @ ".$orderArray[$x][2]."</td><td>$".$total."</td></tr>";
    }
  }
  ...
  $orderList .= '</tbody><tfoot><tr><td colspan="2">Total:</td><td>$'.$grandTotal['Total'].'</td></tr></tfoot></table>';
  if ($success) {
    $to = "jrs40492@gmail.com";
		$from = "admin@mycookiejarandmore.com";
		$subject = 'New Cookie Order';
		$message = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>New Cookie Order</title></head><body style="margin:0px; font-family:Tahoma, Geneva, sans-serif;"><div style="padding:10px; background:rgb(103,58,183); font-size:24px; color:#CCC;">New Cookie Order</div><div style="padding:24px; font-size:17px;">Order Date: '.date('m/d/Y h:i:s a', time()).'<br />Name: '.$name1.' '.$name2.'<br />Number: '.$number.'<br />Email: '.$email.'<br />Address: '.$address.'<br>Order:<br />'.$orderList.'</div></body></html>';
		$headers = "From: $from\n";
